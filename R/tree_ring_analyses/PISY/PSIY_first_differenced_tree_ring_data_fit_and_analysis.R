## Download PISY data and analyse

## API URL for PISY data: 
## https://www.ncdc.noaa.gov/paleo-search/study/search.json?headersOnly=true&dataPublisher=NOAA&dataTypeId=18&locations=Continent>Europe>Northern Europe>British Isles>Ireland|Continent>Europe>Northern Europe>British Isles>United Kingdom&keywords=earth science>paleoclimate>tree-ring>width&species=PISY

setwd("/Users/Staeudchen/Documents/Work_and_Volunteering/Ireland/GMIT/GMIT_Reserach_Assistant_State_Space_Modeling_2016/R/analyses_final_article/R/Tree_rings/RWN_tree_rings/RWN_single_Verror/PISY/first_differenced_PISY")
...
#write.csv2(dat.tree.rings, file="PISY_tree_rings.csv")

dat.tree.rings <- read.csv2(file="/Users/Staeudchen/Documents/Work_and_Volunteering/Ireland/GMIT/GMIT_Reserach_Assistant_State_Space_Modeling_2016/R/analyses_final_article/data/tree_ring_data/PISY/PISY_tree_rings.csv") 
...
for (i in 1:nrow(tree.pairs)){
     ...
     print(c(i, pair.name))
     tmp <- cbind(dat.tree.rings[,pair[1]], dat.tree.rings[,pair[2]])
     colnames(tmp) <- c(pair[1], pair[2])
     tree.pair.ts.list[[pair.name]] <- ts(tmp, start=ring.years[1], end=ring.years[length(ring.years)])
     tmp.index <- index(tree.pair.ts.list[[pair.name]])[rowSums(is.na(tree.pair.ts.list[[pair.name]])) != ncol(tree.pair.ts.list[[pair.name]])] ## find index of removed rows where NA's in both columns
     tree.pair.ts.list[[pair.name]] <- ts(tree.pair.ts.list[[pair.name]][rowSums(is.na(tree.pair.ts.list[[pair.name]])) != ncol(tree.pair.ts.list[[pair.name]]),], start=tmp.index[1], end=tmp.index[length(tmp.index)]) ## remove rows where NA's in both columns
     #tree.pair.ts.list[[pair.name]] <- tree.pair.ts.list[[pair.name]][rowSums(is.na(tree.pair.ts.list[[pair.name]])) != ncol(tree.pair.ts.list[[pair.name]]),] ## remove rows where NA's in all columns
}
length(tree.pair.ts.list)

## take first difference of tree pair time series
...
RWN.build <- function(parm, y.ts, ...){
     ...
     mod$V <- diag(exp(parm[c("V11", "V21")])) # single/shared Verror for among readings of the same otolith
...
## fitting RWN model to tree pairwise comparisons
...
start.parm.RWN <- -log(20/(start.parm.RWN + 10) - 1) # working on the transformed scale between -10 and 10 as BFGS hangs in particular for Vnull

#######################
# i=2
# 
# ## regular
# pair.ts <- tree.pair.ts.list[[pair.name]] 
# pair.ts <- pair.ts[rowSums(is.na(pair.ts)) != ncol(pair.ts),]
# reg.fit <- try(dlmMLE(pair.ts, y.ts=pair.ts, parm=start.parm.RWN, build = RWN.build, hessian=TRUE, control = c(trace=TRUE, maxit=1e4), method="BFGS"))
# corr.est <- tanh((-10 + 20 / (1 + exp(- reg.fit$par["z0"]))) + (-10 + 20 / (1 + exp(- reg.fit$par["delta"]))) * (1:nrow(na.omit(tree.pair.ts.list[[pair.name]]))) / nrow(na.omit(tree.pair.ts.list[[pair.name]])))
# 
# 
# ## first differenced PISY data to remove trend
# (pair.name <- names(diff.tree.pair.ts.list)[2])
# diff.pair.ts <- diff.tree.pair.ts.list[[pair.name]] ## first differenced time series
# #diff.pair.ts <- diff.pair.ts[rowSums(is.na(diff.pair.ts)) != ncol(diff.pair.ts),]
# tmp.fit <- try(dlmMLE(diff.pair.ts, y.ts=diff.pair.ts, parm=start.parm.RWN, build = RWN.build, hessian=TRUE, control = c(trace=TRUE, maxit=1e4), method="BFGS"))
# tmp.fit <- try(dlmMLE(na.omit(diff.pair.ts), y.ts=na.omit(diff.pair.ts), parm=start.parm.RWN, build = RWN.build, hessian=TRUE, control = c(trace=TRUE, maxit=1e4), method="BFGS"))
# eigen(tmp.fit$hessian[c("z0","delta"),c("z0","delta")])
# tmp.est <- tanh((-10 + 20 / (1 + exp(- tmp.fit$par["z0"]))) + (-10 + 20 / (1 + exp(- tmp.fit$par["delta"]))) * (1:nrow(na.omit(diff.tree.pair.ts.list[[pair.name]]))) / nrow(na.omit(diff.tree.pair.ts.list[[pair.name]])))
# plot(tmp.est, type="l", ylim=c(-1,1))
#######################
...
for (i in 1:length(diff.tree.pair.ts.list)){
     ...
     fit.list[[pair.name]] <- try(dlmMLE(pair.ts, y.ts=pair.ts, parm=start.parm.RWN, build = RWN.build, hessian=TRUE, control = c(trace=TRUE, maxit=1e4), method="BFGS"))
     
     if(i %in% seq(1, length(tree.pair.ts.list), by = 10) | i==length(tree.pair.ts.list)){
          saveRDS(fit.list, file="row_removal_if_NA_in_all_columns/test_fits/first_diff_PISY_Verror_start_1e-04.rds")
     }
}
#start 14:08
...
## fit.list <- readRDS("row_removal_if_NA_in_all_columns/first_diff_PISY_tree_rings_single_Verror_RWN_fits_pairwise_comparisons.rds")
## check for error
...
for(i in 1:length(fit.list)){
     ...
     mod.list[[pair.name]] <- try(RWN.build(fit.list[[pair.name]]$par, y.ts=tree.pair.ts.list[[pair.name]]))
     filt.list[[pair.name]] <- dlmFilter(diff.tree.pair.ts.list[[pair.name]], mod=mod.list[[pair.name]])
     #filt.list[[pair.name]] <- dlmFilter(log(tree.pair.ts.list[[pair.name]]), mod=mod.list[[pair.name]]) #log transformed data
...
# saveRDS(results, file="row_removal_if_NA_in_all_columns/test_fits/verror_start_value_1e-04/results_fits_mod_filt_smooth_first_diff_PISY_Verror_1e-04.rds")
# results <- readRDS("row_removal_if_NA_in_all_columns/results_first_diff_PISY_tree_rings_single_Verror_RWN_fits_and_estimates_tree_rings_pairwise_comparisons.rds")
...
fit.list <- readRDS("row_removal_if_NA_in_all_columns/test_fits/verror_start_value_1e-04/results_fits_mod_filt_smooth_first_diff_PISY_Verror_1e-04.rds")[["fit"]]
#########################################
